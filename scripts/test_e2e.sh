...
if curl -sf "${E2E_BASE_URL}/health/ready" >/dev/null; then
  ...
  E2E_BASE_URL="${E2E_BASE_URL}" ${UV_RUN} pytest ${PYTEST_FLAGS} ${E2E_TEST_ARGS:-} -m e2e
  exit 0
fi
...
echo "Starting local API for e2e tests..."
${UV_RUN} python -m uvicorn app.main:app --host "${APP_HOST}" --port "${APP_PORT}" >/tmp/personal-finance-api-e2e.log 2>&1 &
SERVER_PID=$!
